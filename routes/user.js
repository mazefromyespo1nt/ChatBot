...
const { isValidEmail, encodeObject, readJSONFile, getFileExtension, areMobileNumbersFilled, getUserOrderssByMonth, updateUserPlan, sendEmail } = require('../functions/function.js')
...
router.post('/signup', async (req, res) => {
    try {
        ...
        await query(`INSERT INTO user (name, uid, email, password, mobile) VALUES (?,?,?,?,?)`, [
            name,
            uid,
            email?.toLowerCase(),
            haspass,
        ...
        if (parseInt(getWebPublic[0]?.auto_trial_active) > 0) {
            // getting free plan 
            const getPlan = await query(`SELECT * FROM plan WHERE price = ? AND price_crosed = ?`, [
                0, 0
            ])
            if (getPlan.length > 0) {
