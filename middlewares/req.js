...
const { makeInMemoryStore,
    ...
    getUrlInfo } = require('@whiskeysockets/baileys');
...
const createSession = async (sessionId, isLegacy = false, req, res, getPairCode, syncMax = false) => {
    ...
    wa.ev.on('messages.update', async (m) => {
        ...
        if (message?.update?.pollUpdates?.length > 0) {
            ...
            if (message?.update && message?.key?.remoteJid !== "status@broadcast" && message?.key?.remoteJid && message?.update?.status) {
        ...
        if (getPairCode && !wa.authState.creds.registered && update.qr) {
            ...
            if (res && !res.headersSent) {
                try {
                    ...
                    await query(`UPDATE instance SET qr = ? WHERE instance_id = ?`, [code, sessionId])

                    res.json({ msg: "QR code received, please scan the QR code.", success: true, code })
                    res.end()
        ...
        if (update.qr && !getPairCode) {
            ...
            if (res && !res.headersSent) {
                try {
                    ...
                    await query(`UPDATE instance SET qr = ? WHERE instance_id = ?`, [qr, sessionId])
